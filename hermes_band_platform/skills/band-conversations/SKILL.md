...
- Band requires at least one @mention per message, and every mention must carry
  the recipient's Band handle. You choose the recipients: always pass their
  participant UUIDs in `mention_ids`. There is no "mention everyone" default —
  omitting `mention_ids` sends **nothing** and the tool tells you the recipients
  were required.
- Get UUIDs from `band_get_participants` (everyone in the room) or
  `band_find_contact` (resolve a handle/name).
